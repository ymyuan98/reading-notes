...
- Intuitively, the null treatments act as a kind of “anchor” or baseline to help disentangle confounding from causal effects. That is, each $X$ can be used as an auxiliary variable. 
    ...
    $$
    f(y \mid x) = \int_u \tilde{f}(y \mid u, x_{\bar{S}}) \tilde{f}(u \mid x_{S}, x_{\bar{S}}) du
    $$

    given $\tilde{f}(x,u)$ and $f(y|x)$, and that the solution depends on at most $(|\mathcal{C}|-q) / 2$ ones of the confounded treatments, then it must solve 
