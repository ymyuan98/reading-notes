...
-   *Covariate Balancing Propensity Score (CBPS)* method for binary treatment.\
-   Estimate covariates using generalized method of moments (GMM).
...
-   Consistency: $Y_i = Y_i^k$ if $A_i = k$;

-   Ignorability: $\mathbb{E}[Y_i^k|\mathbf{X}_i, A_i] = \mathbb{E}[Y_i^k|\mathbf{X}_i] \equiv m(k,\mathbf{X}_i)$;

-   Positivity: $\operatorname{Pr}(A_i = k|\mathbf{X}_i = \mathbf{x}) \equiv \pi_0(k,\mathbf{x}) > \delta > 0$ for all $k \in \{0, 1, \cdots, K\}$ and all $\mathbf{x}$, where $\pi_0(k, \mathbf{x})$ is named generalized propensity score.
...
-    there exists a (true) value $\boldsymbol{\beta}_0$ with $$\pi(k, \mathbf{x}, \boldsymbol{\beta}_0) = \pi_0(k, \mathbf{x}), $$

-   and there exists (a matrix of coefficients) $\boldsymbol{\alpha} = (\boldsymbol{\alpha}_0^\top, \cdots, \boldsymbol{\alpha}_K^T)^T$ with $$\boldsymbol{\alpha}_k^T \mathbf{B}(k, \mathbf{x}) = m(k, \mathbf{x}),$$ for all $k$ and $\mathbf{x}$. That is, for all (conditional) expectation of potential outcomes...
...
-   Option 2: $$\sum_{i=1}^n \left\{\frac{I(A_i = k)}{\pi(k, \mathbf{X}_i, \boldsymbol{\beta})} - 1 \right\} \mathbf{B}(k, \mathbf{X}_i) = \mathbf{0}
...
-   Ignorability: $\mathbb{E}[Y_i(a)|\mathbf{X}_i, A_i] = \mathbb{E}[Y_i(a)|\mathbf{X}_i]$

-   Positivity: $\pi_0(a, \mathbf{x}) \equiv f_{A|\mathbf{X}}(a, \mathbf{x}) > \delta > 0$ for all $a\in[0,1]$ and all $\mathbf{x}$.

-   write $m(a, \mathbf{x}) \equiv \mathbb{E}[Y_i(a)|\mathbf{X}_i = \mathbf{x}]$.
...
Following the same considerations, there are two different balancing conditions:
...
> Where is randomness? Randomness is implied in the potential outcome $Y(a)$, which is a random function of dose value $a$.\
> And here comes the functional data analysis!!
